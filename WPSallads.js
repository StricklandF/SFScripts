  ...
  try {
    ...
    if (data && data.entries) {
      ...
      if (entries.convert_func_switch) {
        entries.convert_func_switch.pdf_scan_ocr = false;
        entries.convert_func_switch.pure_image_pdf = false;
        entries.convert_func_switch.pdf_scan_ocr_run_switch = false;
      }

      // --- 浮动广告修改 ---
      if (entries.float_ad_config && entries.float_ad_config.float_ad_config && Array.isArray(entries.float_ad_config.float_ad_config.configs)) {
        const configs = entries.float_ad_config.float_ad_config.configs;
        for (const config of configs) {
          if (config.id === "pdf_ocr_scan_global_001") {
            config.display_position = [];
            config.display_scenario = "";
            config.control_codes = [];
            ...
            config.content = {};
            config.button_text = {};
          } else if (config.id === "ai_value_added_001") {
            config.display_position = [];
            config.display_scenario = "";
            config.control_codes = [];
            config.interval_days = 9999;
            config.display_count = 0;
            config.display_duration = 0;
            config.position = "";
            config.priority = 0;
            config.style = "";
            ...
            config.content = {};
            config.button_text = {};
          } else if (config.id === "pdf_edit_pro_001") {
            config.display_position = [];
            config.display_scenario = "";
            config.control_codes = [];
            config.interval_days = 9999;
